...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(centers)):
        point = centers[k]
        small_circles = rg.Circle(point, 10)
        small_circles.fill_color = color
        small_circles.attach_to(window)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(slices)):
        end_point = slices[k]
        line = rg.Line(circle.center, end_point)
        line.color = color
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(50, 100), 100)
    circle.outline_thickness = 3
    polygon(window1, circle, 5, 'yellow', 3)
    window1.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 51, 25, 'midnight blue', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        skips = points[(k + hops_to_next_point) % len(points)]
        line = rg.Line(points[k], skips)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
